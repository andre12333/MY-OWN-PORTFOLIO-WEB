...
export default function ExperienceGala() {
  ...
  return (
    ...
    >
      ...
      <div className="big-text-area">
        <h2 className="bg-text" style={isZh ? { fontSize: "clamp(19rem,51vw,58rem)" } : { fontSize: "clamp(13.2rem,34.3vw,38.3rem)" }}>
          {t("exp_gala", "晚会")}
        </h2>
      </div>
      ...
      <div ref={glassRef} className="glass-panel">
        <div ref={textRef} className="glass-inner">
          <h3 className="card-title">{t(`exp_gala_${chIdx}_title`, "")}</h3>
          <span className="card-role">{t(`exp_gala_${chIdx}_role`, "")}</span>
          <p className="card-desc">{t(`exp_gala_${chIdx}_desc`, "")}</p>
        </div>
      </div>
