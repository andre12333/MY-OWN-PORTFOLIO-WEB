...
const simplexNoise = `vec3 permute(vec3 x) { return mod(((x * 34.0) + 1.0) * x, 289.0); }
float snoise(vec2 v) {
...
export function DitheringShader({
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      const sp = programRef.current;
      if (!ctx || !sp) return;
      const t = (Date.now() - startRef.current) * 0.001 * speed;
      ctx.clear(ctx.COLOR_BUFFER_BIT);
      ctx.useProgram(sp);
      const L = ulRef.current;
      if (L.u_time) ctx.uniform1f(L.u_time, t);
      ...
      if (L.u_shape) ctx.uniform1f(L.u_shape, 4); // wave
      if (L.u_type) ctx.uniform1f(L.u_type, 4); // 8x8
